...
-- Unlike `Traced`, it does not pass conditioning to the transformed monad.
...
-- MH transitions.
mapMonad' :: Monad m => (forall x. m x -> m x) -> Traced' m x -> Traced' m x
mapMonad' t (Traced' m) = Traced' (t . m)
...
-- | Discard the trace.
marginal' :: MonadDist m => Traced' m a -> m a
marginal' = fmap mhAnswer . runTraced'
...
-- This is only applied to the current trace, not to future ones resulting from
-- MH transitions.
mapMonad :: MonadDist m => (forall x. m x -> m x) -> Traced m x -> Traced m x
mapMonad t = Traced . mapMonad' (mapMonadWeightRecorder t) . runTraced
...
mhStep (Traced m) = Traced $ Traced' $ \w -> lift $ do
  (x,s) <- runWeighted $ duplicateWeight $ unTraced' (mhStep' $ mapMonad' resetWeightRecorder m) w
...
-- | Discard the trace.
marginal :: MonadDist m => Traced m a -> m a
marginal = fmap fst . runWeighted . duplicateWeight . marginal' . runTraced
